...
export function createGeneratorTools(): ToolDefinition[] {
  let openscadService: any = null;
  
  const getOpenSCADService = () => {
    if (!openscadService) {
      openscadService = require('../services/openscad.js').getOpenSCADService();
    }
  ...
  return [
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          model: { type: 'object', description: '积木模型数据' },
          tolerance: { type: 'number', description: '公差补偿 (mm)', default: 0.1 },
        },
        required: ['model'],
      },
      returns: { type: 'string', description: 'OpenSCAD 代码' },
      handler: async (params: any) => {
        // 实际由 Generator Agent 处理
        return '// OpenSCAD code generated by GeneratorAgent';
      },
    },
    {
      name: 'export_stl',
      ...
      parameters: {
        type: 'object',
        properties: {
          openscadCode: { type: 'string', description: 'OpenSCAD 代码' },
          modelId: { type: 'string', description: '模型 ID' },
          format: { type: 'string', enum: ['stl', '3mf'], default: 'stl' },
        },
        required: ['openscadCode', 'modelId'],
      },
      returns: { 
        type: 'object',
        properties: {
      ...
      handler: async (params: any) => {
        const service = getOpenSCADService();
        const result = await service.render(params.openscadCode, params.modelId, {
          outputFormat: params.format || 'stl',
        });
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          tolerance: { type: 'number', description: '公差值 (mm)' },
        },
        required: ['openscadCode', 'tolerance'],
      },
      returns: { type: 'string' },
      handler: async (params: any) => {
        // 简单替换公差值
        return params.openscadCode.replace(
          /tolerance = [\d.]+;/,
    ...
    },
    {
      ...
      returns: {
        ...
        properties: {
          ...
          hasSelfIntersections: { type: 'boolean' },
        },
      },
      handler: async (params: any) => {
        // TODO: 实际实现需要网格分析库
